...
class Vertex {
  public position: typeof vec3;
  public normal: typeof vec3;
  public uv: typeof vec2;
        ...
        (item) =>
          new Promise<HTMLImageElement>((resolve) => {
            ...
            img.onerror = () => {
              ...
              // Try loading from assets if URL fails
              if (item.image.startsWith('http')) {
                const assetImg = new Image();
                assetImg.onload = () => resolve(assetImg);
                assetImg.onerror = () => {
                  // If both URL and asset fail, use picsum
                  const fallback = new Image();
                  fallback.crossOrigin = "anonymous";
                  fallback.onload = () => resolve(fallback);
                  fallback.src = `https://picsum.photos/${cellSize}/${cellSize}?random=${Math.random()}`;
                };
                // Try loading from assets folder
                assetImg.src = `/assets/${item.image.split('/').pop()}`;
              } else {
                // If it's already an asset path
                const fallback = new Image();
                fallback.crossOrigin = "anonymous";
                fallback.onload = () => resolve(fallback);
                fallback.src = `https://picsum.photos/${cellSize}/${cellSize}?random=${Math.random()}`;
            ...
            // If the image path starts with 'assets/', prepend with '/'
